...
def test3():
    data = read_data_from_csv('csv8.csv')
    expand_summary(data, 'csv7.csv')


def test4():
    data = read_data_from_csv('csv7.csv')
    expand_summary(data, 'csv8.csv')
...
def expand_review_length(data):
    ...
    data['review_num_length'] = num_words_review
    data.to_csv('csv7.csv', encoding='utf-8', index=False)
...
def expand_summary_length(data):
    ...
    for summary in data.summary:
        words = summary.split()
        num_words = len(words)
        num_words_summary.append(num_words)
    data['summary_num_length'] = num_words_summary
    data.to_csv('csv8.csv', encoding='utf-8', index=False)
